...
#endif
        if (!success) {
            GALog(@"Could not create database queue for path %@", aPath);
            FMDBRelease(self);
            return 0x00;
        }
...
#endif
        if (!success) {
            GALog(@"FMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(__attribute__((noescape)) void (^)(FMDatabase *db))block {
    ...
    dispatch_sync(_queue, ^() {
        ...
        if ([db hasOpenResultSets]) {
            GALog(@"Warning: there is at least one open result set around after performing [FMDatabaseQueue inDatabase:]");
            
#if defined(DEBUG) && DEBUG
            NSSet *openSetCopy = FMDBReturnAutoreleased([[db valueForKey:@"_openResultSets"] copy]);
            for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
                FMResultSet *rs = (FMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
                GALog(@"query: '%@'", [rs query]);
            }
#endif
        }
    });
...
- (NSError*)inSavePoint:(__attribute__((noescape)) void (^)(FMDatabase *db, BOOL *rollback))block {
    ...
    if (_db.logsErrors) GALog(@"%@", errorMessage);

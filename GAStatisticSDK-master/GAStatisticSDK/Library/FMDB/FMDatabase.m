...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
    ...
    return YES;
#else
    GALog(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    GALog(@"Closing leaked statement");
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            GALog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
- (int)busyRetryTimeout {
    GALog(@"%s:%d", __FUNCTION__, __LINE__);
    GALog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    GALog(@"%s:%d", __FUNCTION__, __LINE__);
    GALog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    ...
    if (!query) {
        GALog(@"API misuse, -[FMDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        GALog(@"error on rekey: %d", rc);
        GALog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    GALog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        GALog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                GALog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                GALog(@"DB Query: %@", sql);
                GALog(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                GALog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                GALog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    GALog(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    GALog(@"obj: %@", obj);
                }
    ...
    if (idx != queryCount) {
        GALog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            GALog(@"Error calling sqlite3_step. Query was interrupted (%d: %s) SQLITE_INTERRUPT", rc, sqlite3_errmsg(_db));
            GALog(@"DB Query: %@", sql);
        }
    }
        ...
        if (SQLITE_ERROR == rc) {
            if (_logsErrors) {
                GALog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
                GALog(@"DB Query: %@", sql);
            }
        }
            ...
            // uh oh.
            if (_logsErrors) {
                GALog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
                GALog(@"DB Query: %@", sql);
            }
        }
        else {
            // wtf?
            if (_logsErrors) {
                GALog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
                GALog(@"DB Query: %@", sql);
            }
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            GALog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            GALog(@"DB Query: %@", sql);
        }
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        GALog(@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    if (self.logsErrors) GALog(@"%@", errorMessage);
    return NO;
#endif
}
...
- (BOOL)checkpoint:(FMDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
#else
    GALog(@"sqlite3_wal_checkpoint_v2 unavailable before sqlite 3.7.6. Ignoring checkpoint mode: %d", mode);
    ...
    if(err != SQLITE_OK) {
        ...
        if (self.logsErrors) GALog(@"%@", [self lastErrorMessage]);
